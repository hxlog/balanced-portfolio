...
1. **外币资产先折算**：以新浪 `fx_sina` 的当日人民币汇率为唯一口径，把非人民币资产的 OHLC 折算成人民币计价（`fx_rate` 列记录当日汇率，便于审计）。加密/美元指数/COMEX 黄金不折算——它们的原生计价就是 `/crypto` 看板的展示口径。
2. 把各资产价格重建到统一交易日索引。
3. 对有前后有效价格锚点的内部缺口根据上一个值进行填充。
4. 保留资产上市前的前导空白，由回测的有效起始日处理。
5. 若资产在回测区间尾部停止更新且无法插值填充，构建面板时直接报错。
6. 基于清洗后的价格重新计算收益率，保证协方差、净值、回撤和归因使用同一口径。

这套处理便于跨市场资产对齐，但插值价格不等于可成交价格。涉及实盘执行时，应采用与目标市场一致的交易日历、汇率、复权和停牌处理规则。

> 本地改完清洗口径后请先发布生产再跑本地 `bp_ingest clean`：生产上运行的旧版本会在下一个增量周期把清洗表覆写回它自己的口径。
...
# 站点埋点(可选; 三项都只在 NODE_ENV=production 且 website-id 非空时生效)
BP_UMAMI_WEBSITE_ID=<umami-website-id>
BP_UMAMI_SRC=https://<umami-host>/script.js
# 会话回放/热力图: 独立的第二个 bundle, 不填则从 BP_UMAMI_SRC 推导为同级 recorder.js
# 需 umami >= v3.1.0 且后台该站点开启 Replays; 不需要录屏时留空可省掉 ~190KB 脚本
BP_UMAMI_RECORDER_SRC=https://<umami-host>/recorder.js
```
