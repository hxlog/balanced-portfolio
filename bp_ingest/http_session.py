"""全局 HTTP 会话硬化: 反爬加固(Chrome TLS 指纹 + 持久会话 + 统一浏览器头)。
...
【为何之前无效】akshare 每次 `requests.get` 新建 Session → 无法继承持久会话的指纹。
本模块用**全局持久会话** + **monkeypatch `requests.get/post` 路由到全局会话**,
让所有数据源继承 Chrome 指纹。

【关于 cookie】已移除所有 cookie 注入/预热/刷 cookie 线程。反爬是 IP 级别封锁,
cookie 对封锁无效; 保留 TLS 指纹 + `BP_REQUEST_INTERVAL`/`BP_REQUEST_JITTER` 限速 +
「已最新则本地跳过不发网络请求」的护栏来减少调用次数(见 ingest.py 交易日护栏)。

【仍被掐时】若 curl_cffi 仍 http:000 → IP 信誉限速 → 走 `fetch_with_fallback`
的多源降级链(见 sources.py 的 AGGREGATE_CHAINS); 本机 IP 直连, 不走代理。
HTTP 429/5xx 走长退避(30-60s × 次数, 匹配 EM 5min 封禁窗); 限流统计按 host 计数, 每 5min/退出时打印。
...
def install_hardened_session() -> None:
    """全局硬化 HTTP 会话(幂等): curl_cffi Chrome 指纹 + 持久会话(无 cookie 预热/刷新)。"""
    global _GLOBAL, _installed, _orig_get, _orig_post, _orig_session_init, _orig_session_request
    if _installed:
        return
    ...
    # 兜底: 非 akshare 的 requests.Session() 用户也注入浏览器头 + Retry + Referer
    ...
    _installed = True
    logger.info("HTTP 会话已硬化: %s + 持久会话 (本机 IP 直连, 无代理)", _GLOBAL.engine)
